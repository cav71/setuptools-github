...
def test_check_in_master(git_project_factory):
    project = git_project_factory("check-release").create("0.3.1")

    # this will dump all detectable issues at once
    assert len(checks.check_in_master(project.repo, _testmode=True)) == 1

    # checkout a new branch
    project.checkout(branch="alt-branch")
    assert (
        checks.check_in_master(project.repo)[0].msg
        == "current branch is alt-branch not master branch"
    )

    # swicth back to master and see that fixes
    project.checkout(branch="master", exist_ok=True)

    assert not checks.check_in_master(project.repo)


def test_check_clean(git_project_factory):
    project = git_project_factory("check-release").create("0.3.1")

    # this will dump all detectable issues at once
    assert len(checks.check_clean(project.repo, _testmode=True)) == 1

    # modify the file
    txt = project.initfile.read_text()
    project.initfile.write_text("# booh")

    assert (
        checks.check_clean(project.repo)[0].msg
        == "local modifications present (need to be committed)"
    )

    # restore the contents
    project.initfile.write_text(txt)
